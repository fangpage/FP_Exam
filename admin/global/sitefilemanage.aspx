...
protected override void View()
{
	...
	ViewBuilder.Append("            $(\"#formpost\").submit();\r\n");
	ViewBuilder.Append("        })\r\n");
	ViewBuilder.Append("        PageNav(\"系统站点管理," + echo(rawpath) + "sitemanage.aspx|站点文件管理," + echo(rawpath) + "" + echo(pagename) + "?sitepath=" + echo(m_sitepath) + "\");\r\n");
	ViewBuilder.Append("    })\r\n");
	ViewBuilder.Append("</");
	ViewBuilder.Append("script>\r\n");
	ViewBuilder.Append("</head>\r\n");
	...
	ViewBuilder.Append("            <li style=\"background: url(" + echo(webpath) + "" + echo(sitepath) + "/statics/images/return.gif) 2px 6px no-repeat\"><a href=\"" + echo(reurl) + "\">返回上一级</a></li>\r\n");

	if (m_path=="/")
	{
	ViewBuilder.Append("            <li style=\"float:right; width:auto\"><strong>站点文件管理(" + echo(m_sitepath) + "" + echo(m_path) + ")</strong></li>\r\n");
	}
	else
	{
	ViewBuilder.Append("            <li style=\"float:right; width:auto\"><strong>站点文件管理(" + echo(m_sitepath) + "/" + echo(m_path) + ")</strong></li>\r\n");
	...
	if (m_path=="/")
	{
	ViewBuilder.Append("                <input id=\"filepath" + echo(files["id"]) + "\" name=\"filepath" + echo(files["id"]) + "\" value=\"" + echo(webpath) + "sites/" + echo(m_sitepath) + "" + echo(m_path) + "" + echo(files["name"]) + "\" type=\"hidden\">\r\n");
	}
	else
	{
	ViewBuilder.Append("                <input id=\"filepath" + echo(files["id"]) + "\" name=\"filepath" + echo(files["id"]) + "\" value=\"" + echo(webpath) + "sites/" + echo(m_sitepath) + "/" + echo(m_path) + "" + echo(files["name"]) + "\" type=\"hidden\">\r\n");
	...
	{
	ViewBuilder.Append("                <a href=\"sitefilemanage.aspx?sitepath=" + echo(m_sitepath) + "&path=" + echo(m_path) + "" + echo(files["name"]) + "\">" + echo(files["name"]) + "</a>\r\n");
	}
	...
	{
	ViewBuilder.Append("                <a href=\"sitefileedit.aspx?sitepath=" + echo(m_sitepath) + "&path=" + echo(m_path) + "" + echo(files["name"]) + "\">" + echo(files["name"]) + "</a>\r\n");
	}
	else
	{
	ViewBuilder.Append("                <a href=\"" + echo(webpath) + "sites/" + echo(m_sitepath) + "/" + echo(m_path) + "" + echo(files["name"]) + "\" target=\"_blank\">" + echo(files["name"]) + "</a>\r\n");
	}//end if
	ViewBuilder.Append("            </td>\r\n");
	ViewBuilder.Append("            <td>\r\n");
	...
	ViewBuilder.Append("            </td>\r\n");
	ViewBuilder.Append("            <td>\r\n");
	ViewBuilder.Append("                <a class=\"rename\" node-data=\"" + echo(files["id"]) + "\" href=\"#\">重命名</a>&nbsp\r\n");

	if (files["type"].ToString()=="文件夹")
	{
	ViewBuilder.Append("                <a href=\"sitefilemanage.aspx?sitepath=" + echo(m_sitepath) + "&path=" + echo(m_path) + "" + echo(files["name"]) + "\">打开</a>\r\n");
	}
	...
	{
	ViewBuilder.Append("                <a href=\"sitefileedit.aspx?sitepath=" + echo(m_sitepath) + "&path=" + echo(m_path) + "" + echo(files["name"]) + "\">编辑</a>\r\n");
	}
	else
	{
	ViewBuilder.Append("                <a href=\"" + echo(webpath) + "sites/" + echo(m_sitepath) + "/" + echo(m_path) + "" + echo(files["name"]) + "\" target=\"_blank\">浏览</a>\r\n");
	}//end if
	ViewBuilder.Append("            </td>\r\n");
	ViewBuilder.Append("        </tr>\r\n");
	}//end loop
