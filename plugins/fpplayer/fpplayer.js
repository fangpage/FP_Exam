/*
兼容HTML5-VIDEO(优先)以及FlashPlayer
=====================================================================================================================
*/
function fpplayerConfig() {
	return {
	...
	fpplayer.prototype = {
		...
		*/
		embed: function(c) {
			...
			if(this.vars['config']) {
				this.fpplayerConfig = eval(this.vars['config'] + '()');
			} else {
				this.fpplayerConfig = fpplayerConfig();
			}
		...
		*/
		getVideo: function() {
			...
			//开始构建播放容器
			var playerID = 'fpplayer' + this.randomString();
			...
			if(this.html5Video) { //如果支持HTML5-VIDEO则默认使用HTML5-VIDEO播放器
				...
				var autoLoad = this.fpplayerConfig['config']['autoLoad'];
				var preload = '';
				if(!autoLoad) {
		...
		*/
		...
		interFace: function() {
			...
			*/
			...
			if(this.fpplayerConfig['style']['logo']) {
				if(this.fpplayerConfig['style']['logo']['file']) {
					var logoFile = this.fpplayerConfig['style']['logo']['file'];
		...
		*/
		checkSlideLeft: function(newX, sliderLeft, refer) {
			var timeSA = this.fpplayerConfig['config']['timeScheduleAdjust'];
			switch(timeSA) {
				case 0:
					return false;
					break;
			...
			var keyDown = function(eve) {
				...
				switch(keycode) {
					...
					case 39:
						...
						break;
					case 38:
						now = thisTemp.volume + thisTemp.fpplayerConfig['config']['volumeJump'];
		...
		*/
		fastBack: function() {
			...
			var time = this.time - this.fpplayerConfig['config']['timeJump'];
			if(time < 0) {
				time = 0;
			}
			this.videoSeek(time);
		...
		*/
		...
		changeConfig: function() {
			...
			switch(arg.length) {
				case 2:
					this.fpplayerConfig[arg[0]] = val;
					break;
				case 3:
					this.fpplayerConfig[arg[0]][arg[1]] = val;
					break;
				case 4:
					this.fpplayerConfig[arg[0]][arg[1]][arg[2]] = val;
					break;
				case 5:
					this.fpplayerConfig[arg[0]][arg[1]][arg[2]][arg[3]] = val;
					break;
				case 6:
					this.fpplayerConfig[arg[0]][arg[1]][arg[2]][arg[3]][arg[4]] = val;
					break;
				case 7:
					this.fpplayerConfig[arg[0]][arg[1]][arg[2]][arg[3]][arg[4]][arg[5]] = val;
					break;
				case 8:
					this.fpplayerConfig[arg[0]][arg[1]][arg[2]][arg[3]][arg[4]][arg[5]][arg[6]] = val;
					break;
				case 9:
					this.fpplayerConfig[arg[0]][arg[1]][arg[2]][arg[3]][arg[4]][arg[5]][arg[6]][arg[7]] = val;
					break;
				case 10:
					this.fpplayerConfig[arg[0]][arg[1]][arg[2]][arg[3]][arg[4]][arg[5]][arg[6]][arg[7]][arg[8]] = val;
					break;
				default:
					return;
					break;
			}
			this.sendJS('configChange', this.fpplayerConfig);
		},
		custom: function() {
			if(!this.loaded) {
				return;
		...
		*/
		embedSWF: function() {
			...
			var html = '',
				src = javascriptPath + 'fpplayer.swf';
			id = 'id="' + vid + '" name="' + vid + '" ';
		...
		*/
		getFlashVars: function () {
		    ...
		    z += "stageW=750&stageH=600&showPreviewImage=false&previewImageUrl=&rtmpURI=" + rtmpURI + "&autoHideNav=false&autoHideNavTime=3&fullSizeView=2&showRewind=true&showInfo=false&showFullscreen=true&showScale=true&showSound=true&showTime=true&showCenterPlay=true&showWatermark=false";
			...
			if(typeof(video) == 'object') { //对象或数组
				if(!this.isUndefined(typeof(video.length))) { //说明是数组
					...
					for(i = 0; i < arr.length; i++) {
						...
						if(arr2) {
							if(f != '') {
								f += this.fpplayerConfig['config']['split'];
								d += ',';
								w += ',';
								v['type']+=this.fpplayerConfig['config']['split'];
							}
